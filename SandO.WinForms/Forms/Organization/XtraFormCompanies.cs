...
{
    public partial class XtraFormCompanies : XtraForm
    {
        public XtraFormCompanies()
        {
            InitializeComponent();
        }
        ...
        Company GetGridViewFocusedObject()
        {
            Company company = gridViewMain.GetFocusedRow() as Company;
            return company;
        }

        private void barButtonItemNew_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            XtraFormCompany form = new XtraFormCompany();
            form.ShowDialog();
        ...
        {
            Company company = GetGridViewFocusedObject();
            if (company == null)
            {
                ribbonControl.SetMessage("Güncellenecek şirket seçiniz.", MessageType.Warning);
                return;
            }

            if (company.RecordState != RecordState.Active)
            {
                ribbonControl.SetMessage("Sadece aktif şirketler güncellenebilir.", MessageType.Warning);
                return;
            }

            XtraFormCompany form = new XtraFormCompany(company.Id, FormOpenOption.Update);
            form.ShowDialog();
        ...
        private void barButtonItemView_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (company == null)
            {
                ribbonControl.SetMessage("Görüntülenecek şirket seçiniz.", MessageType.Warning);
                return;
            }


            XtraFormCompany form = new XtraFormCompany(company.Id, FormOpenOption.View);
            form.ShowDialog();
        }

        private void barButtonItemDelete_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            Company focusedObject = GetGridViewFocusedObject();
            if (focusedObject == null)
            {
                ribbonControl.SetMessage("Silinecek şirket seçiniz.", MessageType.Warning);
                return;
            }

            if (focusedObject.RecordState != RecordState.Active)
            {
                ribbonControl.SetMessage("Sadece aktif şirketler silinebilir.", MessageType.Warning);
                return;
            }

            if (XtraMessageBox.Show($"Seçili şirketi {focusedObject.Name} silmek istediğinize emin misiniz?", "Silme Onayı", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
            {
                return;
            }
            ...
            ProgressResult progressResult = companyManager.DeleteCompany(focusedObject.Id);
        ...
        {
            CompanyManager companyManager = new CompanyManager();
            QueryResult<List<Company>> queryResult = companyManager.GetCompanies();
